...
const App = () => {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      let history = priceCache.current.get(symbol) || [];
      history.push(price);
      if (history.length > 200) history = history.slice(-200);
      priceCache.current.set(symbol, history);
      setPriceHistory(history);

      const rsi = calcRSI(history);
      const stoch = calcStoch(history);
      ...
      try {
        const aiRes = await fetch('https://api-inference.huggingface.co/models/google/flan-t5-small', {
          ...
          body: JSON.stringify({
            inputs: `Analyze ${symbol} for trading. RSI=${rsi} Stochastic=${stoch} ADX=${adx}. Recommendation: ${action}. Explain why in 1-2 sentences.`,
      ...
      setAnalysis({
        action, probability, rsi, stoch, adx, macd, tp, sl, entry: price, aiText,
        levels: { support: price * 0.995, resistance: price * 1.015 }
      });
  ...
  const closeTrade = (tradeId: string) => {
    const price = analysis?.entry || priceHistory[priceHistory.length - 1] || 0;
      ...
      <header className="relative z-10 border-b border-purple-500/20 bg-black/90 backdrop-blur p-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-4 text-sm">
            <div className="text-right"><div className="text-gray-500">Винрейт (сессия)</div><div className="font-bold text-green-400">{sessionWinRate}%</div></div>
            <div className="text-right"><div className="text-gray-500">Винрейт (всё)</div><div className="font-bold text-yellow-400">{winRate}%</div></div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4">
          ...
          <div className="flex gap-3 mb-4">
            <div className="relative flex-1">
              <input
                ...
                className="w-full bg-black/60 border border-purple-500/30 rounded-lg px-4 py-3 text-white text-lg"
              />
              {searchSymbol && (
            ...
            <button
              onClick={analyze}
              disabled={loading}
              className={`px-8 py-3 rounded-xl font-bold text-lg transition-all ${loading ? 'bg-gray-700 animate-pulse' : 'bg-gradient-to-r from-purple-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 shadow-lg shadow-purple-500/20'}`}
            >
              ...
              <div className={`p-6 rounded-xl border-2 ${analysis.action === 'LONG' ? 'bg-green-500/10 border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : analysis.action === 'SHORT' ? 'bg-red-500/10 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.3)]' : 'bg-gray-500/10 border-gray-500'}`}>
                ...
                <div className="grid grid-cols-5 gap-2 text-xs mb-4">
                  ...
                  <div className="bg-black/30 rounded p-2 text-center">
                    <div className="text-gray-500">Уровни</div>
                    <div className="text-white text-[10px]">R: ${formatPrice(analysis.levels.resistance)}<br/>S: ${formatPrice(analysis.levels.support)}</div>
                  </div>
                </div>

                {/* Кнопки действий */}
                <div className="flex gap-3">
                  <button onClick={() => openTrade('LONG')} disabled={!sessionId} className={`flex-1 py-3 rounded-xl font-bold text-lg transition-all ${analysis.action === 'LONG' ? 'bg-green-600 hover:bg-green-500 shadow-lg shadow-green-500/30 animate-pulse' : 'bg-gray-700'}`}>
                    🟢 LONG
                  </button>
                  <button onClick={() => openTrade('SHORT')} disabled={!sessionId} className={`flex-1 py-3 rounded-xl font-bold text-lg transition-all ${analysis.action === 'SHORT' ? 'bg-red-600 hover:bg-red-500 shadow-lg shadow-red-500/30 animate-pulse' : 'bg-gray-700'}`}>
                    🔴 SHORT
                  </button>
                  <button onClick={() => {}} disabled={!sessionId} className="flex-1 py-3 rounded-xl font-bold text-lg bg-gray-700 hover:bg-gray-600">
                    ⚪ SKIP
                  </button>
                </div>
        ...
        {analysis && (
          <div className="bg-black/40 rounded-xl border border-purple-500/20 overflow-hidden mb-6">
            ...
            <iframe
              src={`https://s.tradingview.com/widgetembed/?frame_id=tv_${symbol}&symbol=BYBIT:${symbol}&interval=15&hidesidetoolbar=1&symboledit=0&saveimage=0&toolbarbg=000&theme=dark&style=1&locale=ru`}
              width="100%" height="400" frameBorder="0" className="w-full" />
          </div>
        )}
        ...
        <div className="bg-black/40 rounded-xl border border-purple-500/20 overflow-hidden">
          ...
          <div className="divide-y divide-gray-800 max-h-60 overflow-y-auto">
            {trades.length === 0 ? (
              <div className="p-6 text-center text-gray-500 text-sm">Нет сделок</div>
            ) : trades.map(t => (
