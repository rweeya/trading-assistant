...
const calcADX = (p: number[]) => {
  ...
  const s = (d: number[]) => { const k = 2/15; let e = d[0]; for (let i = 1; i < d.length; i++) e = d[i]*k + e*(1-k); return e; };
  const a = s(tr); if (!a) return 0;
  return Math.round(Math.abs(s(pDM) - s(mDM)) / (s(pDM) + s(mDM)) * 100);
};
...
const App = () => {
  ...
  useEffect(() => {
    fetch('https://api.bybit.com/v5/market/tickers?category=spot')
      .then(r => r.json())
      .then(d => {
        if (d.result?.list) setPairs(d.result.list.filter((t: any) => t.symbol.endsWith('USDT')).map((t: any) => t.symbol));
      });
  }, []);
  ...
  const analyzeSymbol = async (sym: string): Promise<Analysis | null> => {
    ...
    let aiText = `${action} сигнал. RSI=${rsi}, Stoch=${stoch}, ADX=${adx}`;
    try {
      ...
      if (aiData?.[0]?.generated_text) aiText = aiData[0].generated_text;
    } catch {}
  ...
  const autoScan = async () => {
    ...
    const batchSize = 5;
    for (let i = 0; i < TOP_PAIRS.length; i += batchSize) {
      const batch = TOP_PAIRS.slice(i, i + batchSize);
      const results = await Promise.all(batch.map(sym => analyzeSymbol(sym)));
      results.forEach((r, idx) => {
        if (r && r.action !== 'SKIP') {
          signals.push({ symbol: batch[idx], action: r.action, probability: r.probability, rsi: r.rsi, stoch: r.stoch, adx: r.adx, macd: r.macd, price: r.entry, tp: r.tp, sl: r.sl, aiReason: r.aiText });
        }
      });
  ...
  useEffect(() => {
    if (mode === 'auto') { autoScan(); const interval = setInterval(autoScan, 120000); return () => clearInterval(interval); }
  }, [mode]);
  ...
  const openTrade = (action: 'LONG' | 'SHORT', sym: string, price: number) => {
    if (!sessionId) return;
    const trade: Trade = { id: Date.now().toString(), symbol: sym, action, entryPrice: price, exitPrice: null, profit: null, time: new Date().toLocaleString(), sessionId };
      ...
      <div className="relative z-10 max-w-6xl mx-auto p-4">
        ...
        {mode === 'manual' && (
          <div className="bg-black/40 rounded-xl p-6 border border-purple-500/20 mb-6">
            ...
            {analysis && <AnalysisCard analysis={analysis} symbol={symbol} sessionId={sessionId} openTrade={(a) => openTrade(a, symbol, analysis.entry)} formatPrice={formatPrice} />}
          </div>
        )}
        ...
        {mode === 'auto' && (
          <div className="bg-black/40 rounded-xl p-6 border border-purple-500/20 mb-6">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h2 className="text-lg font-bold text-purple-300">🤖 АВТО-ПОИСК СИГНАЛОВ</h2>
                <p className="text-xs text-gray-500">Сканирую {TOP_PAIRS.length} пар · Обновление: {lastAutoScan || '...'}</p>
              </div>
              <button onClick={autoScan} disabled={autoScanning} className={`px-4 py-2 rounded-lg text-sm font-bold ${autoScanning ? 'bg-gray-700 animate-pulse' : 'bg-purple-600 hover:bg-purple-500'}`}>
                {autoScanning ? '⏳ Сканирую...' : '🔄 Обновить'}
              </button>
            </div>
            {autoScanning && autoSignals.length === 0 && (
              <div className="text-center py-10">
                <div className="text-4xl animate-pulse mb-3">🔍</div>
                <div className="text-purple-400">AI анализирует рынок...</div>
                <div className="text-xs text-gray-500 mt-2">Поиск лучших сигналов LONG и SHORT</div>
              </div>
            )}
            <div className="space-y-3">
              {autoSignals.map((s, i) => (
                <div key={i} className={`rounded-xl border-2 ${s.action === 'LONG' ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/30'}`}>
                  <div className="p-4">
                    <div className="flex justify-between items-center mb-3">
                      <div>
                        <span className="font-bold text-lg">{s.symbol}</span>
                        <span className={`ml-2 px-2 py-0.5 rounded text-xs font-bold ${s.action === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>{s.action}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className={`text-lg font-bold ${s.probability >= 60 ? 'text-green-400' : 'text-yellow-400'}`}>{s.probability}%</div>
                        <button onClick={() => openTrade(s.action, s.symbol, s.price)} disabled={!sessionId} className={`px-4 py-2 rounded-lg text-sm font-bold ${s.action === 'LONG' ? 'bg-green-600 hover:bg-green-500' : 'bg-red-600 hover:bg-red-500'}`}>
                          {s.action}
                        </button>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-2 mb-3 text-xs">
                      ...
                      <div className="bg-black/30 rounded p-2 text-center">
                        <div className="text-gray-500">Индикаторы</div>
                        <div className="text-white text-[10px]">RSI:{s.rsi} ST:{s.stoch} ADX:{s.adx}</div>
                      </div>
                    </div>
                    <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-3">
                      <div className="text-xs text-purple-400 mb-1">🤖 AI ПРИЧИНА</div>
                      <div className="text-xs text-gray-300">{s.aiReason}</div>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div key={t.id} className="p-3 flex justify-between items-center text-sm">
                ...
                <div className="flex items-center gap-3">
                  {t.exitPrice ? (
                    ...
                    <button onClick={() => closeTrade(t.id, analysis?.entry || 0)} className="px-3 py-1 bg-red-600/50 hover:bg-red-500 rounded text-xs">Закрыть</button>
                  )}
...
const AnalysisCard = ({ analysis, symbol, sessionId, openTrade, formatPrice }: any) => (
  <div className="space-y-4 animate-[fadeIn_0.5s_ease-out]">
    <div className={`p-6 rounded-xl border-2 ${analysis.action === 'LONG' ? 'bg-green-500/10 border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : analysis.action === 'SHORT' ? 'bg-red-500/10 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.3)]' : 'bg-gray-500/10 border-gray-500'}`}>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-right"><div className={`text-3xl font-bold ${analysis.probability >= 60 ? 'text-green-400' : 'text-yellow-400'}`}>{analysis.probability}%</div></div>
      </div>
