...
const getDeepSeekAI = async (sym: string, rsi: number, stoch: number, adx: number, macd: number, action: string, price: number, klines: number[]): Promise<string> => {
  try {
    ...
    const prompt = `Теханализ ${sym}: цена=$${price}, RSI=${rsi}, Stoch=${stoch}, ADX=${adx}, MACD=${macd}, волатильность=${volatility}%, поддержка=$${support.toFixed(4)}, сопротивление=$${resistance.toFixed(4)}, тренд=${trend}. Сигнал: ${action}. Объясни на русском: почему сигнал, уровни, риски, рекомендация TP/SL.`;

    const res = await fetch('https://api.deepseek.com/v1/chat/completions', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${DEEPSEEK_API_KEY}` },
      body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'user', content: prompt }], max_tokens: 120, temperature: 0.3 })
    });
...
const App = () => {
  ...
  const analyzeSymbol = async (sym: string): Promise<Analysis | null> => {
    ...
    if (sig.action !== 'SKIP') ai = `🤖 DeepSeek: ${await getDeepSeekAI(sym, rsi, stoch.k, adx, macd.histogram, sig.action, price, k)}`;
  ...
  const analyze = async () => { setLoading(true); const r = await analyzeSymbol(symbol); if (r) { setAnalysis(r); if (r.action !== 'SKIP' && notifyEnabled) { try { new Notification(`🤖 ${r.action} ${symbol}`, { body: `Вероятность: ${r.probability}% | Вход: $${formatPrice(r.entry)}` }); } catch {} } } setLoading(false); };
  ...
  const closeTrade = (id: string) => { setTrades(prev => prev.map(t => { if (t.id !== id) return t; const cp = currentPrices.get(t.symbol) || t.entryPrice; const p = t.action === 'LONG' ? (cp - t.entryPrice) / t.entryPrice * 100 : (t.entryPrice - cp) / t.entryPrice * 100; return { ...t, exitPrice: cp, profit: Math.round(p * 100) / 100 }; })); showToast('✅ Закрыто'); };
  ...
  const clearAllData = () => { if (confirm('Удалить всё?')) { setTrades([]); setSessionId(null); localStorage.clear(); showToast('🗑 Очищено'); } };
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <header className="relative z-10 border-b border-purple-500/20 bg-black/90 backdrop-blur p-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center flex-wrap gap-4">
          <div className="flex items-center gap-4"><h1 className="text-xl font-bold bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">🤖 AI TRADING ASSISTANT</h1><div className="flex gap-1 bg-black/40 rounded-lg p-1"><button onClick={() => setMode('manual')} className={`px-3 py-1.5 rounded text-xs font-bold ${mode === 'manual' ? 'bg-purple-600 text-white' : 'text-gray-400'}`}>🔍</button><button onClick={() => setMode('auto')} className={`px-3 py-1.5 rounded text-xs font-bold ${mode === 'auto' ? 'bg-purple-600 text-white' : 'text-gray-400'}`}>🤖</button></div><button onClick={() => setNotifyEnabled(!notifyEnabled)} className={`text-xs ${notifyEnabled ? 'text-green-400' : 'text-gray-500'}`}>{notifyEnabled ? '🔔' : '🔕'}</button></div>
          <div className="flex gap-4 text-sm items-center"><div className="text-right"><div className="text-gray-500">Прибыль</div><div className={`font-bold text-lg ${sessionProfit >= 0 ? 'text-green-400' : 'text-red-400'}`}>{sessionProfit >= 0 ? '+' : ''}{sessionProfit}%</div></div><div className="text-right"><div className="text-gray-500">WR</div><div className="font-bold text-green-400">{sessionWinRate}%</div></div><div className="text-right"><div className="text-gray-500">Сделок</div><div className="font-bold text-white">{sessionTrades.length}</div></div><button onClick={resetSession} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded text-xs">🔄</button><button onClick={clearAllData} className="px-3 py-1.5 bg-red-900/50 hover:bg-red-800 rounded text-xs">🗑</button></div>
        </div>
      </header>

      <div className="relative z-10 max-w-6xl mx-auto p-4">
        <div className="flex gap-3 mb-6">
          {!sessionId ? <button onClick={startSession} className="px-6 py-2.5 bg-green-600 hover:bg-green-500 rounded-xl font-bold text-sm animate-pulse">▶ Сессия</button> : <button onClick={endSession} className="px-6 py-2.5 bg-red-600 hover:bg-red-500 rounded-xl font-bold text-sm">⏹ Завершить</button>}
          <a href="https://www.bybit.com" target="_blank" className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-sm">🔗 Bybit</a>
        </div>
          ...
          <div className="bg-black/40 rounded-xl p-6 border border-purple-500/20 mb-6">
            ...
            {analysis && (
              <div className={`p-6 rounded-xl border-2 animate-[fadeIn_0.3s_ease-out] ${analysis.action === 'LONG' ? 'bg-green-500/10 border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.2)]' : analysis.action === 'SHORT' ? 'bg-red-500/10 border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'bg-gray-500/10 border-gray-500'}`}>
                ...
                <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4 mb-4"><div className="text-xs text-purple-400 mb-1">🤖 DeepSeek AI — полный тех. анализ</div><div className="text-sm text-gray-300 whitespace-pre-line">{analysis.aiText}</div></div>
            ...
            <div className="space-y-3">
              {autoSignals.map((s, i) => (
                <div key={i} className={`rounded-xl border hover:scale-[1.01] transition-all ${s.action === 'LONG' ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/30'}`}>
                  <div className="p-4"><div className="flex justify-between items-center mb-3"><div><span className="font-bold text-lg">{s.symbol}</span><span className={`ml-2 px-2 py-0.5 rounded text-xs font-bold ${s.action === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>{s.action}</span></div><div className="flex items-center gap-3"><div className={`text-lg font-bold ${s.probability >= 60 ? 'text-green-400' : 'text-yellow-400'}`}>{s.probability}%</div><button onClick={() => openTrade(s.action, s.symbol, s.price)} className={`px-4 py-2 rounded-lg text-sm font-bold ${s.action === 'LONG' ? 'bg-green-600 hover:bg-green-500' : 'bg-red-600 hover:bg-red-500'}`}>{s.action}</button></div></div>
        ...
        {analysis && mode === 'manual' && (
          <div className="bg-black/40 rounded-xl border border-purple-500/20 overflow-hidden mb-6">
            <div className="p-3 bg-purple-950/20 border-b border-purple-500/20 flex justify-between items-center"><span className="text-sm font-bold text-purple-300">📈 ГРАФИК {symbol}</span><button onClick={() => setShowChart(!showChart)} className="text-xs text-gray-400 hover:text-white">{showChart ? 'Скрыть' : 'Показать'}</button></div>
