...
const TOP_PAIRS = [
  ...
  'TRUUSDT', 'VRAUSDT', 'WAXPUSDT', 'CFXUSDT', 'MASKUSDT'
];
...
// Прогноз цены на N минут вперёд
const predictPrice = (klines: number[], minutes: number): { price: number; direction: 'up' | 'down' } => {
  const price = klines[klines.length - 1];
  const emaFast = calcEMA(klines, 9);
  const emaSlow = calcEMA(klines, 21);
  const macd = calcMACD(klines);
  const atr = calcADX(klines) * price / 10000;
  
  // Базовый прогноз по тренду
  let direction: 'up' | 'down' = price > emaFast ? 'up' : 'down';
  let projectedChange = (emaFast - emaSlow) / emaSlow * 100;
  
  // Усиление от MACD
  if (macd.histogram > 0 && macd.crossed === 'up') projectedChange *= 1.5;
  else if (macd.histogram < 0 && macd.crossed === 'down') projectedChange *= 1.5;
  
  // Масштабируем на время
  const timeMultiplier = minutes / 15;
  const finalChange = projectedChange * timeMultiplier * 0.3;
  
  const predictedPrice = price * (1 + finalChange / 100);
  
  return { price: predictedPrice, direction: finalChange > 0 ? 'up' : 'down' };
};
...
const getDeepSeekAI = async (sym: string, rsi: number, stoch: number, adx: number, macdData: any, action: string, price: number, klines: number[], fundingRate: number): Promise<string> => {
  try {
    ...
    const prompt = `${sym}: $${price}, RSI=${rsi}, Stoch=${stoch}, ADX=${adx}, ${cross}, vol=${vol}%, S=$${support}, R=$${resistance}, ${trend}, ${fr}. Сигнал: ${action}. Краткая рекомендация.`;
...
interface Signal { symbol: string; action: 'LONG' | 'SHORT'; probability: number; rsi: number; stoch: number; adx: number; macd: number; price: number; tp: number; sl: number; aiReason: string; predictions: { min: number; price: number; direction: string }[]; }
interface Analysis { action: 'LONG' | 'SHORT' | 'SKIP'; probability: number; rsi: number; stoch: number; adx: number; macd: number; tp: number; sl: number; entry: number; aiText: string; predictions: { min: number; price: number; direction: string }[]; }
...
const App = () => {
  ...
  const analyzeSymbol = async (sym: string): Promise<Analysis | null> => {
    ...
    return { action: sig.action, probability: sig.probability, rsi, stoch: stoch.k, adx, macd: macd.histogram, tp, sl, entry: price, aiText: ai, predictions };
  };
  ...
  const autoScan = async () => {
    ...
    for (let i = 0; i < TOP_PAIRS.length; i += 5) {
      ...
      res.forEach((r, idx) => { if (r && r.action !== 'SKIP') sigs.push({ symbol: b[idx], action: r.action, probability: r.probability, rsi: r.rsi, stoch: r.stoch, adx: r.adx, macd: r.macd, price: r.entry, tp: r.tp, sl: r.sl, aiReason: r.aiText, predictions: r.predictions }); });
  ...
  const openTrade = (action: 'LONG' | 'SHORT', sym: string, price: number) => { if (!sessionId) { showToast('⚠️ Начни сессию!'); return; } setTrades(prev => [{ id: Date.now().toString(), symbol: sym, action, entryPrice: price, exitPrice: null, profit: null, time: new Date().toLocaleTimeString(), sessionId }, ...prev]); showToast(`✅ ${action} ${sym}`); window.open('https://pocketoption.com', '_blank'); };
              ...
              <div className={`p-6 rounded-xl border-2 ${analysis.action === 'LONG' ? 'bg-green-500/10 border-green-500' : analysis.action === 'SHORT' ? 'bg-red-500/10 border-red-500' : 'bg-gray-500/10 border-gray-500'}`}>
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                  {analysis.predictions.map(p => (
                    <div key={p.min} className={`bg-black/40 rounded-lg p-3 text-center border ${p.direction === 'up' ? 'border-green-500/30' : 'border-red-500/30'}`}>
                      <div className="text-xs text-gray-500">Через {p.min} мин</div>
                      <div className={`text-lg font-bold ${p.direction === 'up' ? 'text-green-400' : 'text-red-400'}`}>${formatPrice(p.price)}</div>
                      <div className="text-xs text-gray-500">{p.direction === 'up' ? '📈' : '📉'}</div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-3">
              {autoSignals.map((s, i) => (
                <div key={i} className={`rounded-xl border hover:scale-[1.01] transition-all ${s.action === 'LONG' ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/30'}`}>
                  <div className="p-4"><div className="flex justify-between items-center mb-3"><div><span className="font-bold text-lg">{s.symbol}</span><span className={`ml-2 px-2 py-0.5 rounded text-xs font-bold ${s.action === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>{s.action}</span></div><div className="flex items-center gap-3"><div className={`text-lg font-bold ${s.probability >= 70 ? 'text-green-400' : 'text-yellow-400'}`}>{s.probability}%</div><button onClick={() => openTrade(s.action, s.symbol, s.price)} className={`px-4 py-2 rounded-lg text-sm font-bold ${s.action === 'LONG' ? 'bg-green-600' : 'bg-red-600'}`}>{s.action === 'LONG' ? 'ВВЕРХ' : 'ВНИЗ'}</button></div></div>
                    <div className="grid grid-cols-4 gap-2 mb-3 text-xs">
                      {s.predictions.slice(0, 4).map(p => (
                        <div key={p.min} className={`bg-black/30 rounded p-2 text-center border ${p.direction === 'up' ? 'border-green-500/20' : 'border-red-500/20'}`}>
                          <div className="text-gray-500">{p.min}м</div>
                          <div className={`font-bold ${p.direction === 'up' ? 'text-green-400' : 'text-red-400'}`}>${formatPrice(p.price)}</div>
                        </div>
                      ))}
                    </div>
